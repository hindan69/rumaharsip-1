(function (factory) {
  ...
  const basicBarChartInit = () => {
   ...
   const horizontalBarChartInit = () => {
    const { getColor, getData } = window.phoenix.utils;
    const $chartEl = document.querySelector(
      '.echart-horizontal-bar-chart-mod'
    );

    if ($chartEl) {
      fetch('/satker/datachart') // URL sesuai route di CI4
      .then(response => response.json())
      .then(result => {
          const months = result.map(item => item.name);
      ...
      const getDefaultOptions = () => ({
        ...
        yAxis: {
          ...
          boundaryGap: true,
          axisLabel: {
            formatter: value => value.substring(0, 350),
            show: true,
            color: getColor('quaternary-color'),
            margin: 15
          },
  ...
  const pieChartInit = () => {
    ...
    if ($chartEl) {
      const userOptions = getData($chartEl, 'echarts');
      const chart = window.echarts.init($chartEl);
      fetch('/chart/getChartData') // Ganti dengan URL endpoint yang benar
      .then(response => response.json())
      .then(data => {
          const chartData = data.map(item => ({
              ...
              name: item.name,
              itemStyle: {
                  color: getColor(item.name.toLowerCase()) // Sesuaikan dengan kebutuhan
              }
          }));
              ...
              series: [
                  {
                      ...
                      center: ['50%', '55%'],
                      data: chartData, // Menggunakan data dari database
                      emphasis: {
                          itemStyle: {
                              shadowBlur: 10,
                              shadowOffsetX: 0,
  ...
  docReady(horizontalBarChartInit2);
